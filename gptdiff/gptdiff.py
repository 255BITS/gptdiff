...
def apply_diff(project_dir, diff_text):
    ...
    This version uses pre-checks on hunk headers and file hash comparisons to detect modifications.

    Returns:
        True if the patch was applied and resulted in a change,
        False otherwise.
    """
    from pathlib import Path
    ...
    def parse_diff_per_file(diff_text):
        ...
        Returns a list of tuples (file_path, patch) for each file.
        """
        diffs = []
        file_path = None
        current_diff = []
    ...
    # Write the diff text to a temporary patch file.
    ...
    # Pre-check: Verify that each hunk header references a valid line in the target file.
    # For each hunk header of the form @@ -<start>[,<len>] +... @@, check that <start> is <= number of lines in the file.
    hunk_pattern = re.compile(r"@@ -(\d+)(?:,(\d+))? \+")
    for file_path, patch in file_patches:
        target_file = Path(project_dir) / file_path
        if target_file.exists():
            file_lines = target_file.read_text().splitlines()
            num_lines = len(file_lines)
            for match in hunk_pattern.finditer(patch):
                removal_start = int(match.group(1))
                removal_len = int(match.group(2)) if match.group(2) else 1
                if removal_len > 0 and removal_start > num_lines:
                    print(f"Pre-check failed: hunk removal start {removal_start} exceeds file line count {num_lines} in {target_file}")
                    return False
    ...
    # Perform a dry run to verify the patch applies cleanly.
    dry_run = subprocess.run(
        ["patch", "--dry-run", "-p1", "--remove-empty-files", "--fuzz=0", "--input", str(diff_file)],
        cwd=project_dir,
        capture_output=True,
        text=True
    )
    dry_run_output = dry_run.stdout + dry_run.stderr
    if dry_run.returncode != 0 or "FAILED" in dry_run_output:
        print("Dry run failed or hunk application failed. The patch may not apply as expected:")
        print(dry_run_output)
        return False
    if "patching file" not in dry_run_output and "checking file" not in dry_run_output:
        print("Dry run did not report any file patching. The patch may not apply as expected:")
        print(dry_run_output)
        return False

    print("Dry run successful. Applying patch...")
    result = subprocess.run(
        ["patch", "-p1", "--remove-empty-files", "--fuzz=0", "--input", str(diff_file)],
        cwd=project_dir,
        capture_output=True,
        text=True
    )
    result_output = result.stdout + result.stderr
    if result.returncode != 0 or "FAILED" in result_output:
        print("Patch application failed:")
        print(result_output)
        return False

    # Verify that a change occurred by comparing file hashes.
    any_change = False
    for file_path, patch in file_patches:
        target_file = Path(project_dir) / file_path
        if "+++ /dev/null" in patch:
